"""
agents/vision_mock_payloads.py — PassportAI

Deterministic mock payloads for VisionAgent demo_mock mode.

These payloads are returned when VisionAgent is initialised without a
GemmaClient (client=None). They exist to make the Gradio demo and the
test suite runnable without a live Ollama server.

Each function returns a fully-typed AgentPayload matching the contracts
defined in agents/contracts.py.
...
def build_textiles_mock_payload(image_url: str | None = None) -> AgentPayload:
    """Return a deterministic mock payload for a textile product (canvas tote bag)."""
    return {
        "domain_data": {
            "espr_core": {
...
def build_batteries_mock_payload(image_url: str | None = None) -> AgentPayload:
    """Return a deterministic mock payload for a battery product (rechargeable pack)."""
    return {
        "domain_data": {
            "espr_core": {
...
def build_electrical_mock_payload(image_url: str | None = None) -> AgentPayload:
    """Return a deterministic mock payload for an electrical appliance (washing machine)."""
    return {
        "domain_data": {
            "espr_core": {
...
def build_insufficient_data_payload(image_url: str | None = None) -> AgentPayload:
    """Return a payload indicating the image lacks sufficient evidence for extraction."""
    return {
        "domain_data": {
            "espr_core": {
                "product_name": None,
